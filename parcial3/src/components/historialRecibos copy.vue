         ...
         <h1>
           ...
           <h2>Todos los recibos</h2> 
            <ul>
                ...
                <p class="pListas">Recibo pagado:</p> <p v-if="rec.Pagado == true">Pagado</p><p v-if="rec.Pagado != true">NoPagado</p><hr/>
...
export default {
    ...
    methods: {
        ...
        async mostrarUltimoRecibo(){ //este. acano jjsjsjs x2
           ...
           // eslint-disable-next-line no-unused-vars
           const orden=ref.orderBy("FechaPago", 'desc').limit(1).get()
            .then(snapshot => {
                        if (snapshot.empty) {
